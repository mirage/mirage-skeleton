...
module Make
  ...
  let tls key_ro certificate_ro =
    ...
    let fold acc (name, _) = match Mirage_kv.Key.to_string name with
      ...
      | _ ->
        ...
        Lwt.return_ok ((Mirage_kv.Key.to_string name, certificates) :: acc) in
    ...
    let fold acc (name, _) = match Mirage_kv.Key.to_string name with
      ...
      | _ ->
        ...
        >>= fun acc -> Lwt.return_ok ((Mirage_kv.Key.to_string name, key) :: acc) in
  ...
  let run_with_tls ~ctx ~authenticator ~tls http_server tls_port tcpv4v6 =
    ...
    HTTP_server.init ~port:tls_port tcpv4v6 >|= Paf.serve alpn_service >>= fun (`Initialized th0) ->
